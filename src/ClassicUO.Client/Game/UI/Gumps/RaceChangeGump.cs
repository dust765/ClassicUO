...
namespace ClassicUO.Game.UI.Gumps
{
    internal class RaceChangeGump : Gump
    {
        ...
        private void BuildHairStyles(int x, int y)
        {
            ...
            Add
            (
                new Label(UOFileManager.Current.Clilocs.GetString(selectedRace == RaceType.GARGOYLE ? 1112309 : 3000121), unicode, hue, font: font)
                {
                    X = x + 1,
                    Y = y
                }
            ...
            if (!isFemale && selectedRace != RaceType.ELF)
            {
                ...
                Add
                (
                    new Label(UOFileManager.Current.Clilocs.GetString(selectedRace == RaceType.GARGOYLE ? 1112511 : 3000122), unicode, hue, font: font)
                    {
                        X = x + 1,
                        Y = y
                    }
        ...
        private class CustomColorPicker : Control
        {
            ...
            public CustomColorPicker(Layer layer, int label, ushort[] pallet, int rows, int columns)
            {
                ...
                Add
                (
                    new Label(UOFileManager.Current.Clilocs.GetString(label), unicode, hue, font: font)
                    {
                        X = 0,
                        Y = 0
                    }
