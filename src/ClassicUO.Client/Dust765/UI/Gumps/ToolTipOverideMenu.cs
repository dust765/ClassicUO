...
namespace ClassicUO.Dust765.UI.Gumps
{
    internal class ToolTipOverideMenu : Gump
    {
        ...
        private void BuildGump()
        {
            ...
            Add(background);
            int y = 0;
            OptionsGump.SettingsSection section = new OptionsGump.SettingsSection("Tooltip override settings", WIDTH);
        ...
        private Area NewAreaSection(int keyLoc, int y)
        {
            ...
            _searchText.TextChanged += (s, e) =>
            {
                Debounce(searchTextRevision, 1500, () =>
                {
                    ...
                    if (_searchText.Text == tVal)
                    {
                        ...
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _searchText.ScreenCoordinateX, Y = _searchText.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _formatText.TextChanged += (s, e) =>
            {
                Debounce(formatTextRevision, 1500, () =>
                {
                    ...
                    if (_formatText.Text == tVal)
                    {
                        ...
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _formatText.ScreenCoordinateX, Y = _formatText.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _min1.TextChanged += (s, e) =>
            {
                Debounce(min1Revision, 1500, () =>
                {
                    ...
                    if (_min1.Text == tVal && int.TryParse(_min1.Text, out int val))
                    {
                        data.Min1 = val;
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _min1.ScreenCoordinateX, Y = _min1.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _max1.TextChanged += (s, e) =>
            {
                Debounce(max1Revision, 1500, () =>
                {
                    ...
                    if (_max1.Text == tVal && int.TryParse(_max1.Text, out int val))
                    {
                        data.Max1 = val;
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _max1.ScreenCoordinateX, Y = _max1.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _min2.TextChanged += (s, e) =>
            {
                Debounce(min2Revision, 1500, () =>
                {
                    ...
                    if (_min2.Text == tVal && int.TryParse(_min2.Text, out int val))
                    {
                        data.Min2 = val;
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _min2.ScreenCoordinateX, Y = _min2.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _max2.TextChanged += (s, e) =>
            {
                Debounce(max2Revision, 1500, () =>
                {
                    ...
                    if (_max2.Text == tVal && int.TryParse(_max2.Text, out int val))
                    {
                        data.Max2 = val;
                        data.Save();
                        UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _max2.ScreenCoordinateX, Y = _max2.ScreenCoordinateY - 20 });
                    }
                });
            };
            ...
            _itemLater.OnOptionSelected += (s, e) =>
            {
                ...
                data.Save();
                UIManager.Add(new SimpleTimedTextGump("Saved", Color.LightGreen, TimeSpan.FromSeconds(1)) { X = _itemLater.ScreenCoordinateX, Y = _itemLater.ScreenCoordinateY - 20 });
            };
